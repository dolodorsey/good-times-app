-- Keep the customer Home/live venue feed image-diverse.
-- Explore still exposes the complete directory; this only deduplicates the
-- capped live venue payload by hero image so one photo is not repeated across
-- multiple venue aliases/locations before other useful places appear.
  ...
  select jsonb_build_object(
    ...
    'venues', coalesce((
      ...
      from (
        ...
        where image_rank = 1
        order by
          (hero_image like '%/brand-graphics/good_times/graphics/LOCATION_IMAGES/%') desc,
          is_culture_pick desc,
          is_black_owned desc,
