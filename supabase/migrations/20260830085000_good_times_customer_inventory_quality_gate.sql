...
'Customer-safe public venue inventory: verified/usable experiences only; generic grocery/supermarket/liquor-store records are excluded and unique photography is optional.';

comment on function public.gt_public_live_inventory(text,date,integer,integer) is
'Customer live inventory with verified venue coverage, unique-media preservation, and non-experience transportation records removed before ranking.';
