...
Deno.serve(async req=>{
  ...
  try{
    ...
    const {data:session,error:sessionError}=await admin.from('gt_recommendation_sessions').insert({auth_id:user.id,city_slug:city,action,raw_query:rawQuery,parsed_intent:{surface:clean(body?.surface||'home',80),vibes},formula_key:formula.formula_key,formula_version:formula.version,maturity_stage:maturity,exploration_rate:explorationRate,candidate_count:Math.max(rawItems.length,Number(body?.candidate_count)||rawItems.length),result_count:rawItems.length,itinerary_id:itineraryId,duration_ms:Math.max(0,Date.now()-started)}).select('id').single()
