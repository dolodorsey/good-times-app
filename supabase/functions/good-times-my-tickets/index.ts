...
function cors(origin:string|null){const value=allowedOrigin(origin)?origin!:'https://thegoodtimesworldwide.com';return {'Access-Control-Allow-Origin':value,'Access-Control-Allow-Headers':'authorization, content-type, apikey','Access-Control-Allow-Methods':'GET, OPTIONS','Vary':'Origin'}}
...
async function qrImage(token:string){const svg=await QRCode.toString(token,{type:'svg',errorCorrectionLevel:'M',margin:1,width:256});return `data:image/svg+xml;charset=utf-8,${encodeURIComponent(svg)}`}
Deno.serve(async req=>{
  ...
  const email=String(user?.email||'').trim().toLowerCase();const userId=String(user?.id||'').trim()
  if(!email||!userId)return json(401,{ok:false,error:'GOOD TIMES account identity is incomplete'},origin)
  const serviceUrl=Deno.env.get('SUPABASE_URL');const serviceKey=Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')
  ...
  const [byUser,byEmail]=await Promise.all([admin.from('event_tickets').select(select).eq('user_id',userId).order('purchased_at',{ascending:false}).limit(100),admin.from('event_tickets').select(select).eq('user_email',email).order('purchased_at',{ascending:false}).limit(100)])
  if(byUser.error||byEmail.error){console.error('[GOOD TIMES ticket wallet]',{byUser:byUser.error?.message,byEmail:byEmail.error?.message,userId});return json(503,{ok:false,error:'Ticket wallet is temporarily unavailable'},origin)}
  const merged=new Map<string,any>();for(const ticket of [...(byUser.data||[]),...(byEmail.data||[])])merged.set(ticket.id,ticket)
  const sorted=[...merged.values()].sort((a,b)=>String(b.purchased_at||'').localeCompare(String(a.purchased_at||'')))
  const tickets=await Promise.all(sorted.map(async ticket=>{const credential=String(ticket.qr_token||'').trim();const generated=credential?await qrImage(credential):null;const {qr_token,...safeTicket}=ticket;return {...safeTicket,qr_code:ticket.qr_code||generated}}))
