...
const PRODUCTION_ORIGINS=new Set(['https://thegoodtimesworldwide.com','https://www.thegoodtimesworldwide.com'])
const PREVIEW_ORIGIN=/^https:\/\/good-times-[a-z0-9-]+-dr-dorseys-projects\.vercel\.app$/i

function allowedOrigin(origin:string|null){return Boolean(origin&&(PRODUCTION_ORIGINS.has(origin)||PREVIEW_ORIGIN.test(origin)))}
function cors(origin:string|null){
  const allowed=allowedOrigin(origin)?origin!:'https://thegoodtimesworldwide.com'
  return {'Access-Control-Allow-Origin':allowed,'Access-Control-Allow-Headers':'authorization, content-type, apikey','Access-Control-Allow-Methods':'GET, OPTIONS','Vary':'Origin'}
}
...
Deno.serve(async req=>{
  ...
  const select='id,event_id,brand_key,event_series,event_name,event_date,event_city,venue,venue_name,venue_address,tier,price,quantity,total,status,qr_code,ticket_number,checked_in,checked_in_at,purchased_at,source,payment_provider'
  ...
  const tickets=[...merged.values()].sort((a,b)=>String(b.purchased_at||'').localeCompare(String(a.purchased_at||'')))
