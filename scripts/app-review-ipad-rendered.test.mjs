  ...
  test(`${vp.name} guest discovery uses the iPad canvas`,{skip,timeout:45000},async()=>{
    ...
    try{
      ...
      const geometry=await page.evaluate(()=>{
        ...
        const guest=document.querySelector('.gt4-profile-auth')?.getBoundingClientRect()
        const profile=document.querySelector('.gt4-profile')?.getBoundingClientRect()
        ...
        return {
          ...
          profile:profile&&{top:profile.top,bottom:profile.bottom,width:profile.width},
      ...
      assert.ok(geometry.profile&&geometry.profile.top>=-2&&geometry.profile.bottom<=geometry.vh+2,`guest profile sheet is outside the iPad viewport; geometry=${JSON.stringify(geometry)}`)
      ...
      assert.match(geometry.text,/Browse freely/i)
      assert.match(geometry.text,/Sign in or create account/i)
