...
async function installRoutes(ctx){
  ...
  await ctx.route('**/rest/v1/gt_user_follows**',route=>route.fulfill(json([])))
  await ctx.route('**/rest/v1/gt_alert_preferences**',route=>route.fulfill(json([])))
  await ctx.route('**/rest/v1/gt_alert_queue**',route=>route.fulfill(json([])))
