...
for(const vp of[{width:320,height:740},{width:390,height:844},{width:430,height:932},{width:834,height:1194}]){
 test(`Home/Discover ${vp.width}: reference density retains all controls`,{skip,timeout:50000},async()=>{
  ...
  try{
   ...
   await ctx.route('**/api/**',r=>{const p=new URL(r.request().url()).pathname;return r.fulfill(json(p==='/api/health'?{ok:true,service:'good-times',customer_ready:true,content_ready:true}:p.startsWith('/api/data')?{ok:true,connected:true,city:'atlanta',events:[EVENT],venues:VENUES}:{ok:true}))})
   ...
   const feature=await page.locator('.gt5-card-feature').boundingBox(),dock=await page.locator('.gt5-nav').boundingBox()
