...
for(const vp of [{name:'phone',width:390,height:844},{name:'tablet',width:834,height:1194}]){
 test(`${vp.name}: Tonight and Upcoming render the correct fixture set`,{skip,timeout:45000},async()=>{
  ...
  try{
   ...
   await page.getByText('Tonight in Atlanta',{exact:true}).waitFor()
   assert.deepEqual(await page.locator('.gt-ux-rows .gt-ux-card h3').allTextContents(),['Evening fixture'])
   await page.screenshot({path:path.join(OUT,`time-fixture-${vp.name}-tonight.png`)})
   await modes.filter({hasText:/^Upcoming$/}).click()
   await page.getByText('Coming up in Atlanta',{exact:true}).waitFor()
   const upcomingTitles=await page.locator('.gt-ux-rows .gt-ux-card h3').allTextContents()
