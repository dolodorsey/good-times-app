...
for(const width of [320,390,430,834])test(`customer enhancements ${width}: page layout, planning controls and Shake`,{skip:!BASE||!chromium,timeout:60000},async()=>{
 ...
 await ctx.route('**/api/**',r=>r.fulfill(json(new URL(r.request().url()).pathname==='/api/health'?{ok:true,customer_ready:true,content_ready:true,service:'good-times'}:{ok:true,city:'atlanta',connected:true,events:[],venues})))
