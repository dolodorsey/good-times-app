...
test('builds compile canonical source instead of rewriting it with taxonomy patches', () => {
  for (const name of ['build','ios:build','android:build']) {
    assert.doesNotMatch(packageJson.scripts[name], /apply-taxonomy-vs-guard\.mjs/)
  }
})
