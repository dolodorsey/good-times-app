...
test('signed-in navigation keeps the six primary customer surfaces', () => {
  mustContain(live, ["['home','⌂','Now']","['dates','▦','Dates']","['build','✦','Build']","['plans','≋','Plans']","['explore','◇','Explore']","['vault','▣','Vault']"])
})

test('city switching persists to the authenticated profile and reloads live inventory', () => {
  mustContain(live, ['const switchCity=async nextCity=>','updatePreferences(session.user.id,{last_city:nextCity}','await loadCity(nextCity)'])
})
...
test('ticket and reservation conversions feed the learning loop', () => {
  mustContain(personalization, ["trackConversion('ticket_click'","trackConversion('reservation_click'","eventName==='ticket_click'?'ticket_clicked'","eventName==='reservation_click'?'reservation_clicked'"])
})

test('plans can be routed and shared and the Vault exposes tickets', () => {
  mustContain(liveUx, ['function planDirectionsUrl(plan)','const sharePlan=async plan=>',"vaultType==='tickets'",'goodtimes:open-tickets'])
})
...
test('members have a visible account center with permanent deletion', () => {
  ...
  mustContain(account, ['GOOD TIMES ACCOUNT','Delete account permanently','deleteAccount(session)','/privacy.html','/support.html'])
})
...
test('desktop web root is not trapped in a phone-width shell', () => {
  ...
  assert.equal(indexHtml.includes('max-width:430px;min-height:100dvh'), false)
})
...
test('member bridge controls cannot become viewport-sized overlays', () => {
  ...
  mustContain(shellCss, ['.gt-premium-experience>.gt-connect-fab','button[aria-label="Open GOOD TIMES account"]','button[aria-label="Tickets and paid experiences"]'])
  assert.equal(/\.gt-premium-experience > \*\s*\{/.test(premiumCss), false)
})

test('current launch media is mounted without bypassing the canonical shell', () => {
  mustContain(main, ['GT_CURRENT_LOGO','GT_CURRENT_HOME','GT_CURRENT_ANIMATION','good-times-shell.css'])
})
