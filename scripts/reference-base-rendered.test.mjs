...
for(const viewport of [{width:390,height:844},{width:834,height:1194},{width:1440,height:900}]){
 test(`reference-base nine screens ${viewport.width}`,{skip,timeout:120000},async()=>{
  ...
  try{
   ...
   await page.locator('.gt-ux-chips button').filter({hasText:/^Drafts/}).click();await page.locator('.gt-ux-my-plans button').first().waitFor();await page.locator('.gt-ux-my-plans button').first().click();await page.locator('.gt5-itinerary').waitFor();assert.equal(await page.locator('.gt5-timeline section').count(),3)
   ...
   await page.locator('.gt-ux-library-tabs button').filter({hasText:/^Saved/}).click();await page.getByRole('heading',{name:venues[0].name,exact:true}).click();await page.locator('.gt5-detail').waitFor();await capture('08-venue-detail','.gt5-overlay');await page.locator('.gt5-detail-back').click()
