...
const CONTENT_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXAiLCJyZWYiOiJkemxtdHZvZHB5aGV0dmVrdGZ1byIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNzY5NTg0ODY0LCJleHAiOjIwODUxNjA4NjR9.qmnWB4aWdb7U8Iod9Hv8PQAOJO3AG0vYEGnPS--kfAo'
...
async function fetchRows(path, label) {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`${label} returned HTTP ${response.status}: ${text.slice(0, 200)}`)
...
export default async function handler(request, response) {
  ...
  try {
    if (mode === 'directory') {
      ...
      const query = [
        `v_gt_venue_taxonomy_directory?select=${DIRECTORY_SELECT}`,
    ...
    for (const row of memberships) {
      if (row.hero_image && row.category_key && !categoryImages.has(row.category_key)) categoryImages.set(row.category_key, row.hero_image)
      if (row.hero_image && row.subcategory_key && !subcategoryImages.has(row.subcategory_key)) subcategoryImages.set(row.subcategory_key, row.hero_image)
    }
    ...
    for (const row of subcategories) {
      ...
      grouped.get(row.category_key).push({
        ...
        count: subcategoryCounts.get(row.subcategory_key)?.size || 0,
        image: subcategoryImages.get(row.subcategory_key) || null,
      })
    }
    ...
    return send(response, 200, {
      ...
      categories: categories.map(category => ({
        ...
        count: categoryCounts.get(category.category_key)?.size || 0,
        image: categoryImages.get(category.category_key) || null,
