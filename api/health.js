...
function headers(key) {
  const requestHeaders = { apikey: key, Accept: 'application/json' }
  // Modern sb_publishable_* keys identify the public application and are not JWTs.
  // Keep Authorization only for the legacy JWT anon key; user JWTs belong there when signed in.
  if (!key.startsWith('sb_publishable_')) requestHeaders.Authorization = `Bearer ${key}`
