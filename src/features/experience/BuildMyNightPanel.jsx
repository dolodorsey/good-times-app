...
const VIBES = [
  ['Grown & sexy','gt-cat-mood-bougie.webp'],['High energy','gt-cat-mood-turnt.webp'],
  ...
  ['Food first','gt-cat-dining.webp'],['Something different','gt-cat-mood-explore.webp'],
]
...
export default function BuildMyNightPanel({ cityName, events = [], venues = [], busy = false, onBuild }) {
  ...
  const route=useMemo(()=>{
    const first=firstPool[(routeShift)%Math.max(firstPool.length,1)]||anchor
    const main=anchor||events[(routeShift)%Math.max(events.length,1)]||venues[0]
    const last=latePool[(routeShift+1)%Math.max(latePool.length,1)]||venues[1]||main
  ...
  return <section className="gt2-builder gt2-builder-live" aria-labelledby="gt-build-night-title">
    ...
    {step===3&&<section className="gt2-builder-stage"><div className="gt2-builder-stage-title"><div><span>4 · EDIT THE ROUTE</span><h3>Your night, assembled live.</h3></div><small>Not feeling it? Swap the route before GOOD TIMES checks final timing.</small></div><div className="gt2-builder-live-route">{route.map((item,index)=><article key={`${itemKey(item)}:${index}`}><b>0{index+1}</b>{imageFor(item)?<img src={imageFor(item)} alt=""/>:<i>GT</i>}<span><small>{['START','MAIN MOVE','LATE MOVE'][index]}</small><strong>{itemName(item)}</strong><em>{itemMeta(item)||cityName}</em></span></article>)}</div><button className="gt2-builder-swap" type="button" onClick={()=>setRouteShift(value=>value+1)}>↻ Swap all three suggestions</button></section>}
