...
import GoodTimesPlannerStudio from './GoodTimesPlannerStudio.jsx'
import {HomeView,DirectoryView,ProfileLibrary} from './GoodTimesUXScreens.jsx'
import {UX_NAV,canonicalTab} from './good-times-ux-model.js'
import './good-times-ux.css'
...
function AdSlot({placement,city,onPlan}){
...
const ENTERTAINMENT_EVENT_CATEGORIES=new Set(['comedy_performing_arts','arts_museums_culture','games_interactive','attractions_experiences','seasonal_holiday','family_kids'])
...
export default function GoodTimesCommandAppV4({onAuth=null}){
  ...
  const[city,setCity]=useState('atlanta'),[tab,setTab]=useState(()=>canonicalTab(new URL(window.location.href).searchParams.get('view')||'home')),[loading,setLoading]=useState(true),[query,setQuery]=useState(''),[intent,setIntent]=useState('')
  ...
  const[conciergeText,setConciergeText]=useState(''),[conciergeBusy,setConciergeBusy]=useState(false),[conciergeResult,setConciergeResult]=useState(null),[conciergeMessage,setConciergeMessage]=useState(''),[planMode,setPlanMode]=useState('click'),[planIntent,setPlanIntent]=useState('')
  ...
  const conciergeLock=useRef(false),scrollPositions=useRef({})
  const[visitedViews,setVisitedViews]=useState(()=>new Set([canonicalTab(new URL(window.location.href).searchParams.get('view')||'home')]))
  const[directoryQueries,setDirectoryQueries]=useState({entertainment:'',venues:''})
  useEffect(()=>{const back=()=>{const next=canonicalTab(new URL(window.location.href).searchParams.get('view')||'home');setTab(next);setVisitedViews(rows=>new Set([...rows,next]));requestAnimationFrame(()=>document.querySelector('.gt5-main')?.scrollTo({top:scrollPositions.current[next]||0,behavior:'instant'}))};window.addEventListener('popstate',back);return()=>window.removeEventListener('popstate',back)},[])
  ...
  const runConcierge=async(text=conciergeText,action='recommend')=>{const clean=String(text||'').trim();if(!clean||conciergeBusy||conciergeLock.current)return null;conciergeLock.current=true;setConciergeBusy(true);setConciergeMessage('');void recordTasteSignal({entityType:'category',entityId:clean.slice(0,120),signalType:'concierge_select',city,metadata:{action}},session);try{const result=hardenRecommendationResult(await askGoodTimesConcierge({query:clean,action,city},session));setConciergeResult(result);setConciergeMessage(result?.message||'Here are the strongest current options.');if(result?.itinerary){setPlans(rows=>[result.itinerary,...rows.filter(x=>x.id!==result.itinerary.id)]);setSelectedPlan(result.itinerary);setToast('Your draft plan is ready.')}return result}catch(error){setConciergeMessage(error.message||'I could not verify a strong answer from current data.')}finally{conciergeLock.current=false;setConciergeBusy(false)}}
  const goTab=id=>{const next=canonicalTab(id);scrollPositions.current[tab]=document.querySelector('.gt5-main')?.scrollTop||0;if(next==='radar'){returnTab.current=PRIMARY_TABS.has(tab)?tab:'home'}setTab(next);setVisitedViews(rows=>new Set([...rows,next]));if(next!==tab){const url=new URL(window.location.href);url.searchParams.set('view',next);history.pushState({gtView:next},'',`${url.pathname}${url.search}${url.hash}`)}requestAnimationFrame(()=>document.querySelector('.gt5-main')?.scrollTo?.({top:scrollPositions.current[next]||0,behavior:'instant'}))}
  const exploreUX=(scope,text='')=>{setDirectoryQueries(current=>({...current,[scope]:text}));goTab(scope)}
  const planUX=(text='')=>{setConciergeText(text);setPlanMode(text?'ask':'click');goTab('plan')}
  ...
  return <div className="gt5-app gt-ux-shell" data-screen={tab}>
    ...
    <main className="gt5-main">
      {tab==='home'&&<HomeView mode={homeMode} onMode={setHomeMode} events={rankedEvents} today={today} upcoming={upcomingEvents} venues={homeVenues} savedKeys={savedKeys} onEvent={openEvent} onVenue={openVenue} onSave={toggleSave} onExplore={exploreUX} onPlan={planUX} heroImage={screenEditorialMedia(city,'home')} plans={plans} onPlanOpen={setSelectedPlan}/>}

      {['entertainment','venues'].map(scope=>visitedViews.has(scope)&&<div key={scope} hidden={tab!==scope}><DirectoryView scope={scope} visible={tab===scope} query={directoryQueries[scope]} onQuery={value=>setDirectoryQueries(current=>({...current,[scope]:value}))} savedKeys={savedKeys} onEvent={openEvent} onVenue={openVenue} onSave={toggleSave} heroImage={screenEditorialMedia(city,'discover')}/></div>)}

      {visitedViews.has('plan')&&<div hidden={tab!=='plan'}><GoodTimesPlannerStudio session={session} mode={planMode} onMode={setPlanMode} venues={homeVenues} onOpen={openVenue} onBuild={runConcierge} busy={conciergeBusy} initialText={conciergeText} heroImage={screenEditorialMedia(city,'plan')}/></div>}
      ...
      {tab==='profile'&&<section className="gt5-screen gt5-profile"><Hero eyebrow="YOUR ACCOUNT" title={profile?.full_name||session?.user?.user_metadata?.full_name||'Your Good Times'} subtitle={session?'GT Concierge Member':'Browse freely. Sign in when you want to save, follow and personalize.'} image={heroMedia} className="gt5-compact-hero"/>
        <ProfileLibrary plans={plans} saved={saved} events={events} venues={venues} savedKeys={savedKeys} onPlanOpen={setSelectedPlan} onCreate={()=>planUX('')} onEvent={openEvent} onVenue={openVenue} onSave={toggleSave}/>
