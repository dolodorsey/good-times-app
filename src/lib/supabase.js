...
const CANONICAL_KHG_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXAiLCJyZWYiOiJkemxtdHZvZHB5aGV0dmVrdGZ1byIsInJvbGUiOiJhbm9uIiwiaWF0IjoxNzY5NTg0ODY0LCJleHAiOjIwODUxNjA4NjR9.qmnWB4aWdb7U8Iod9Hv8PQAOJO3AG0vYEGnPS--kfAo'
...
function validPublicKey(value) {
  return typeof value === 'string' && (value.startsWith('eyJ') || value.startsWith('sb_publishable_')) && value.length > 40
}

export const GT_SUPABASE_URL = validProjectUrl(env.VITE_GT_SUPABASE_URL, 'czocqfaovfpjweayniuw')
  ? env.VITE_GT_SUPABASE_URL
  : CANONICAL_GT_URL
export const GT_SUPABASE_ANON_KEY = validPublicKey(env.VITE_GT_SUPABASE_ANON_KEY)
  ? env.VITE_GT_SUPABASE_ANON_KEY
  : CANONICAL_GT_ANON_KEY
export const KHG_SUPABASE_URL = validProjectUrl(env.VITE_KHG_SUPABASE_URL, 'dzlmtvodpyhetvektfuo')
  ? env.VITE_KHG_SUPABASE_URL
  : CANONICAL_KHG_URL
export const KHG_SUPABASE_ANON_KEY = validPublicKey(env.VITE_KHG_SUPABASE_ANON_KEY)
  ? env.VITE_KHG_SUPABASE_ANON_KEY
  : CANONICAL_KHG_ANON_KEY
...
async function readJson(baseUrl, anonKey, query, fetchImpl = globalThis.fetch) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (!response.ok) {
        ...
        lastError = new Error(`Supabase ${response.status}: ${body || response.statusText}`)
      ...
      if (!Array.isArray(payload)) {
        lastError = new Error('Supabase returned a non-array payload')
        break
      }
  ...
  if (typeof window !== 'undefined') {
    window.__GOOD_TIMES_BACKEND_ERROR__ = {
      url,
...
export async function checkGoodTimesBackend(fetchImpl = globalThis.fetch) {
  ...
  return {
    gt: profileProbe.length > 0,
    khg: contentProbe.length > 0,
    connected: profileProbe.length > 0 && contentProbe.length > 0,
  }
}
